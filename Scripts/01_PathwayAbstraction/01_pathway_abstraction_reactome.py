...
for owl_file in sorted(filelist):
    ...
    iscomponentof = pd.read_csv(f"../../Results/PathwayAbstraction/01_TopPathways/{counter:02d}_IsAComponentOf.csv", sep=",", header=0)
    new_col = ["abstraction:IsAComponentOf"]*len(iscomponentof)
    iscomponentof.insert(loc=1, column="interaction", value=new_col)
    iscomponentof.to_csv(f"../../Results/PathwayAbstraction/01_TopPathways/{counter:02d}_IsAComponentOf.csv", sep=",", header=True, index=False)
    ...
    nextsteppathway.to_csv(f"../../Results/PathwayAbstraction/01_TopPathways/{counter:02d}_NextStepPathway.csv", sep=",", header=True, index=False)
